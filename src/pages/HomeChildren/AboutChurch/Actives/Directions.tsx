...
const Directions = ({title}: {title: string}) => {
    ...
    return (
        <div
            className={`transition-all duration-500 ease-in-out flex flex-col h-full ${active === 0 ? "w-full" : "w-full c_md:w-1/4"}`}
            onMouseEnter={() => setActive(0)}
            onMouseLeave={() => setActive(null)}
        >
            <p className="text-2xl c_md:text-3xl mb-4 c_md:mb-10 text-black dark:text-white font-semibold">{title}</p>
            <div id="maps-buffer" className="p-2 c_md:p-5 h-full">
                <NaverMaps className="h-96 c_md:h-[80vh] c_lg:h-[60vh] w-full pb-14 c_md:pb-0 rounded-lg overflow-hidden" />
            </div>
        </div>
    )
}
